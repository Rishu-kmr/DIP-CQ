...
def make_color_map_for_global_image_matching():
    ...
    while(i<m-sample_size):
        ...
        while(j<n-sample_size):
            ...
            sd = np.std(padded_orig_img[row-2:row+3,col-2:col+3,0])
            ...
            color_map.append([intensi,sd,padded_orig_img[row][col][0],padded_orig_img[row][col][1],padded_orig_img[row][col][2]])
            #sds_map.append(sd)
...
def find(error,color_map):
    index = -1;
    dist = sys.maxsize
    for i,rows in enumerate(color_map):
        sds = error-(0.5*rows[0]+0.5*rows[1])
...
def form_colorization_of_gray_image_by_global_coloring():
    ...
    while(r_ind<padded_gray_img.shape[0]-5):
        ...
        while(c_ind<padded_gray_img.shape[1]-5):
            ...
            neighborhood_sd = np.std(padded_gray_img[r_ind-2:r_ind+3,c_ind-2:c_ind+3,0])
            neighborhood = 0.5*neighborhood_intensity+0.5*neighborhood_sd
            index = find(neighborhood,color_map)
            new_img[r_ind-5][c_ind-5][0] = intensity
            new_img[r_ind-5][c_ind-5][1] = color_map[index][3]
            new_img[r_ind-5][c_ind-5][2] = color_map[index][4]
            c_ind = c_ind+1
        r_ind = r_ind+1
